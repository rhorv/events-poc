...
Set up the Confluent Schema registry:
9. Run ``make confluent-schema-registry-start``  
   (Wait for a few minutes for it to start, get a cup of tea/coffee - at this point you will need a lot of ram as well)
   When it's all up you can view the schema registry at "http://localhost:8081", and the control-center at "http://localhost:9021"
10. Run ``make confluent-schema-registry-setup``  
    This will load the generic schemas into the schema registry for both serialization family,  
    you can verify it at "http://localhost:8081/schemas" (You should see 2 schemas "rjs1-generic" and "hav1-generic").
...
## How to switch schema provider implementations

The POC now has an adapter to connect to the confluent schema registry (running as a docker container).  
By default the IOC container is configured to use it. However there is another implementation provided, which is a simple file based provider.

In order to use the confluent schema registry you should have these alieses defined in your container config:  
...
To use the simple file based schema provider, comment out the other ones, and use:  
